...
def botmain(botConn):
    ...
    while True:
        ...
        # This is what happens when there's no new data from the socket
        except socket.error:
            pass
        ...
        if not(linebuffer):
            # Is it really a good idea to use "type" for this? Seems questionable since it's a builtin name. 
            etype = 'periodic'
            if etype in listenerregistry:
                for function in listenerregistry[etype]:
                    l = function[0]
                    target = function[1]
